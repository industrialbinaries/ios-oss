...
internal final class MessageThreadEnvelopeTest: XCTestCase {
  func testDecoding() throws {
    let result = try MessageThreadEnvelope.decodeJSON([
      "participants": [
        [
          "id": 1,
          "name": "Blob",
    ...
    XCTAssertEqual(1, result.participants.count)
    XCTAssertEqual(1, result.participants.first?.id)
    XCTAssertEqual(0, result.messages.count)
  }
}
